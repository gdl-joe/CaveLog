...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `is_active`     TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'Zugang aktiv (0 = gesperrt)',
  `prefs`         JSON COMMENT '{"theme":"dark","layout":"cards","diffMode":"bars"}',
  `invite_token`  VARCHAR(64) NULL COMMENT 'Einladungslink für Erstanmeldung / Passwort-Reset',
  `invite_expires` DATETIME NULL COMMENT 'Ablauf des Einladungslinks',
  `invited_by`    INT NULL,
...
CREATE TABLE IF NOT EXISTS `caves` (
  ...
  `notes`          TEXT,
  `cover_path`     VARCHAR(500) NULL COMMENT 'Höhlen-Titelbild (groß)',
  `cover_thumb`    VARCHAR(500) NULL COMMENT 'Höhlen-Titelbild (Thumbnail)',
...
CREATE TABLE IF NOT EXISTS `photos` (
  ...
  `large_path`  VARCHAR(500) COMMENT 'Groß 1200px (Kacheln)',
  `full_path`   VARCHAR(500) COMMENT 'Vollbild, längste Kante 2048px',
  `caption`     TEXT,
